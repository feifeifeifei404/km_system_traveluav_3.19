            ...
            while True:
                try:
                    ...
                    for idx, camera_name in enumerate(cameras):
                        ...
                        # 2. 修改：使用 cv2 解码 PNG 数据
                        image_bytes = np.frombuffer(rgb_resp.image_data_uint8, dtype=np.uint8)
                        image = cv2.imdecode(image_bytes, cv2.IMREAD_COLOR) # 解码为 BGR
                        
                        # 如果原来的代码没有 reshape 错误，这一步解码后 shape 自动就是 (H, W, 3)
                        # 如果需要确保是 RGB，可以加一句：image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                        # 但 AirSim 默认通常是 BGR，且你的模型可能已经适应了，先保持原样 BGR 即可。
        ...
        def _getImages(airsim_client: airsim.VehicleClient):
            ...
            while True:
                try:
                    ImageRequest = []
                    for camera_name in cameras:
                        # === 修改 1: 开启压缩 compress=True ===
                        ImageRequest.append(airsim.ImageRequest(camera_name, airsim.ImageType.Scene, pixels_as_float=False, compress=True))
                        # 深度图保持 compress=False
                        ImageRequest.append(airsim.ImageRequest(camera_name, airsim.ImageType.DepthPerspective, pixels_as_float=True, compress=False))
