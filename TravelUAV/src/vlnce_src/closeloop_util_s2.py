...
class EvalBatchState:
    ...
    def update_from_env_output(self, outputs):
        ...
        for i in range(self.batch_size):
            ...
            #     self.dones[i] = True
            #     self.termination_reasons[i] = "失败：碰撞或卡死 (Failure: Collision/Stuck)"
            #     print("输出结束原因：")
            #     print(self.termination_reasons[i])
            
            # --- 检查并输出结束原因的逻辑 ---
            # 首先检查是否因为“卡住”而结束
            # 我们只在 episode 在此之前尚未结束的情况下检查
            ...
            # 如果没有因为“卡住”而结束，再检查是否因为“深度图碰撞”而结束
            # 通过对比检查前后的状态来判断
            ...
            # 最后，如果以上都不是，但 episode 结束了，那一定是环境自己终止的
            if dones_from_env[i]:
                # 注意：我们无法仅凭 done=True 信号区分是成功还是超时，所以给一个通用原因
                self.termination_reasons[i] = "结束：由模拟环境直接终止 (可能成功、超时或环境内置碰撞)"
                print(f"环境 {i} 结束，原因: {self.termination_reasons[i]}")
        ...
        for i in range(self.batch_size):
            ...
            if self.predict_dones[i] and not self.skips[i]:
                ...
                if self.oracle_success[i] and self.early_end[i]:
                    self.dones[i] = True
                    self.termination_reasons[i] = "成功：距离>20m，但裁判的仿真环境（Oracle）判定成功 (Success: Oracle Success)"
                    print("输出结束原因：") 
                    print(self.termination_reasons[i])
                elif self.success[i]:
                    self.dones[i] = True
                    self.termination_reasons[i] = "成功：到达目标 (Success: Reached Target)" # <--- 新增
                    print("输出结束原因：") 
                    print(self.termination_reasons[i])
    ...
    def check_batch_termination(self, t):
        for i in range(self.batch_size):
            if t == args.maxWaypoints:
                self.dones[i] = True
                print("输出结束原因：") 
                self.termination_reasons[i] = "失败：超时 (Failure: Timeout)" # <--- 新增
                print(self.termination_reasons[i])
            ...
            if self.dones[i] and not self.skips[i]:
                ...
                self.final_metrics[i] = {
                    ...
                    "is_success": self.success[i],
                    "total_tokens": total_tokens,
                    "tokens_per_step": self.tokens_per_step[i] # 也可选择性保存
                }
                # === 新增代码结束 ===
