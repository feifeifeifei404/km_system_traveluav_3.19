...
env.setdefault("EVAL_SAVE_DIR", os.path.join(root_dir, "result", "eval_closeloop", "eval_Modular"))

cmd = [
    sys.executable,
    os.path.join(root_dir, "src", "vlnce_src", "eval.py"),
    "--run_type", "eval",
    "--name", "TravelLLM_Modular_Bridge_Test",
    ...
    "--eval_save_path", os.path.join(root_dir, "result", "eval_closeloop", "eval_Modular"),
    ...
    "--eval_json_path", os.path.join(root_dir, "Dataset", "dataset_split", "data", "uav_dataset","unseen_scene_valset.json"),
...
# 透传本脚本收到的额外命令行参数给 eval.py（例如 --use_complexity_switch / --use_budget_forcing
# / --complexity_evaluator / --complexity_fixed_score / --complexity_cooldown_seconds 等）
extra_args = sys.argv[1:]
if extra_args:
    print("[INFO] 透传额外参数到 eval.py:", " ".join(extra_args))
    cmd.extend(extra_args)

print("[INFO] Running all Modular tasks from seen_valset")
print("[INFO] Eval JSON:", os.path.join(root_dir, "Dataset", "dataset_split", "data", "uav_dataset", "unseen_scene_valset.json"))
