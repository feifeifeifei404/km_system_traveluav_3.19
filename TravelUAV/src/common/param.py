...
class CommonArguments:
    ...
    use_budget_forcing: bool = field(
        default=False,
        metadata={
            "help": "Enable iterative waypoint refinement based on the 's1' paper's test-time scaling."
        },
    )
    num_refinement_steps: int = field(
        default=1,
        metadata={
            "help": "Number of refinement steps for budget forcing (e.g., 1 means one extra 'Wait' step)."
        },
    )

    num_parallel_thoughts: int = field(
        default=3,
        metadata={
            "help": "Number of initial candidates to generate in parallel."
        }
    )
    ...
    record_dir: str = field(
        default='./debug_data',
        metadata={"help": "数据记录保存目录"}
    )
    max_record_eps: Optional[int] = field(
        default=None,
        metadata={"help": "最多记录多少个episode（None表示全部）"}
    )
