...
//            cout << YELLOW << " -- [Fsm] ReplanOnce failed." << RESET << endl;
    ...
    void Fsm::callMainFsmOnce() {
        ...
        if (cur_t - last_print_t > 1.0) {
            last_print_t = cur_t;
            if ((!robot_state_.rcv || (ros_ptr_->getSimTime() - robot_state_.rcv_time) > 1.0)) {
    ...
    void Fsm::setGoalPosiAndYaw(const Vec3f &p, const Quatf &q) {
        ...
        auto click_point = p;
        // 注释掉 click_height 强制覆盖，使用目标点自带的 Z 值
        // if (cfg_.click_height > -5) {
        //     click_point.z() = cfg_.click_height;
        // }

        // 临时注释：不检查目标是否被占据，直接使用目标点
        // 如果需要恢复检查，取消下面的注释并注释掉 gi_.goal_p = click_point;
        ...
        //     return;
        // }
        gi_.goal_p = click_point;  // 直接使用目标点
        cout << GREEN << " -- [Fsm] Get goal at " << RESET << gi_.goal_p.transpose() << endl;

        if ((robot_state_.p - gi_.goal_p).norm() < 0.5) {
            cout << YELLOW << " -- [Fsm] Too close to goal (< 0.5m), skip this target." << RESET << endl;
            return;
        }
