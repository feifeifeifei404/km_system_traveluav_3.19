...
class EvalBatchState:
    ...
    # 作者本身的失败检测碰撞逻辑
    # 这里的逻辑问题在于：
    # `一个常见的“卡死”场景是什么？是无人机一头撞向一堵墙或者一个障碍物，然后想继续往前飞但飞不动，导致它离目标的距离不再减小甚至开始增加。
    # 在这种最常见的卡死场景下，会发生什么：
    # check_collision_by_depth 先运行: 当无人机离墙很近时，深度传感器会立刻检测到即将发生碰撞。这个函数会立即将 self.dones[i] 设置为 True，并且 termination_reasons 被设置为 "Failure: Collision (Depth Sensor)"。
    # target_distance_increasing_for_10frames 后运行: 当代码执行到卡死检测的 if 语句时，它的第一个条件 not self.dones[i] 已经是 False 了！
    # 结果: 因为第一个条件不满足，整个 if 语句块被直接跳过。卡死的逻辑根本没有机会执行。任务已经因为“碰撞”而被终止了。
    # 结论： 你的碰撞检测逻辑“抢跑”了，它总是在卡死检测之前就把任务结束掉了。因此，所有因为撞墙而卡死的案例，最终都被归类为了“碰撞”。`
    def update_from_env_output(self, outputs):
        ...
        dones_before_check = list(self.dones)
        
        # 1. 检查深度图碰撞
        self.collisions, self.dones = self.assist.check_collision_by_depth(self.episodes, observations, collisions_from_env, dones_from_env)
        
        for i in range(self.batch_size):
            if i in self.envs_to_pause: continue
            
            for j in range(len(observations[i])):
                self.episodes[i].append(observations[i][j])
            self.distance_to_ends[i].append(self._calculate_distance(observations[i][-1], self.target_positions[i]))
            
            if self.dones[i] and not dones_before_check[i]: # 如果是本轮刚结束的
                self.termination_reasons[i] = "Failure: Collision (Depth Sensor)"

            # 2. 检查卡死（连续远离目标）
            if not self.dones[i] and target_distance_increasing_for_10frames(self.distance_to_ends[i]):
                self.collisions[i] = True
                self.dones[i] = True
                self.termination_reasons[i] = "Failure: Stuck (Increasing Distance)"
    ...
    #     1) 先追加观测与距离历史
    #     2) 优先判定“卡死”（连续远离目标）
    #     3) 再用深度图进行碰撞检测，仅对尚未结束的实例
    #     """
    #     observations, dones_from_env, collisions_from_env, self.oracle_success = [
    ...
    #     # 2) 【再次检查】深度图碰撞，仅对尚未结束的任务进行检查
    #     # 注意：这里的输入 self.dones 已经可能被“卡死”逻辑修改过
    #     collisions_after_stuck_check, dones_after_stuck_check = self.assist.check_collision_by_depth(
    #         self.episodes, observations, self.collisions, self.dones
    #     )

    #     # 更新状态并设置原因
    #     self.collisions = collisions_after_stuck_check
    #     for i in range(self.batch_size):
    #         # 只有当任务是因为这次碰撞检测而刚刚结束时，才把原因归为碰撞
    #         if not self.dones[i] and dones_after_stuck_check[i]:
    #             self.termination_reasons[i] = "Failure: Collision (Depth Sensor)"

    #     self.dones = dones_after_stuck_check
    ...
    def update_metric(self):
        for i in range(self.batch_size):
            ...
            if self.predict_dones[i] and not self.skips[i]:
                ...
                if self.success[i]:
                    self.dones[i] = True
                    self.termination_reasons[i] = "Success: Reached Target"
                elif self.oracle_success[i] and self.early_end[i]:
                    self.dones[i] = True
                    # 这是一个特殊的成功，也记录下来
                    self.termination_reasons[i] = "Success: Oracle Success (Early Stop)"
    
    # === 【修改点 6】: 大幅修改 check_batch_termination 实现片段保存 ===
    def check_batch_termination(self, t):
        NUM_STEPS_TO_SAVE_ON_FAILURE = 5000  # 定义失败时保存的步数

        for i in range(self.batch_size):
            # 1. 检查超时
            if t == args.maxWaypoints and not self.dones[i]:
                self.dones[i] = True
                self.termination_reasons[i] = "Failure: Timeout"

            # 2. 处理所有已结束的任务
            if self.dones[i] and not self.skips[i]:
                is_successful = self.success[i] or self.oracle_success[i]
                
                if is_successful:
                    # --- 成功案例处理 ---
                    prex = 'success_' if self.success[i] else 'oracle_'
                    new_traj_name = prex + self.ori_data_dirs[i].split('/')[-1]
                    new_traj_dir = os.path.join(args.eval_save_path, new_traj_name)
                    
                    print(f"Task {i} succeeded ({self.termination_reasons[i]})! Saving full trajectory...")
                    save_to_dataset_eval(self.episodes[i], new_traj_dir, self.ori_data_dirs[i])
                else:
                    # --- 失败案例处理 ---
                    reason_slug = self.termination_reasons[i].lower().replace(' ', '_').replace(':', '').replace('(', '').replace(')', '')
                    prex = f'fail_{reason_slug}_'
                    new_traj_name = prex + self.ori_data_dirs[i].split('/')[-1]
                    new_traj_dir = os.path.join(args.eval_save_path, new_traj_name)
                    
                    print(f"Task {i} failed ({self.termination_reasons[i]})! Saving last {NUM_STEPS_TO_SAVE_ON_FAILURE} steps...")
                    last_n_steps_data = self.episodes[i][-NUM_STEPS_TO_SAVE_ON_FAILURE:]
                    save_to_dataset_eval(last_n_steps_data, new_traj_dir, self.ori_data_dirs[i])

                self.skips[i] = True
                self.envs_to_pause.append(i) # 任务结束后暂停，不再交互
