...
# (从 load_model 到 project_this_state2target_state_axis 的所有函数保持不变)
# ...
def load_model(args):
    model_path = os.path.expanduser(args.model_path)
    model_name = get_model_name_from_path(model_path)
    tokenizer, model, image_processor, _ = load_pretrained_model(model_path, args.model_base, model_name, vision_tower=args.vision_tower)
...
# === 【修改点 1】: 修改 preprocess_multimodal ===
def preprocess_multimodal(
    ...
    # 新增一个参数来接收上一步的航点字符串，用于迭代精炼
    previous_waypoint_str: Optional[str] = None
) -> Dict:
    """
    Process image token's representation and dynamically build the prompt,
    including refinement instructions if available.
    """
    for source in sources:
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value']:
                ...
                if previous_waypoint_str:
                    refinement_instruction = f'\n\nPrevious Decision: Your last proposed waypoint was approximately {previous_waypoint_str}. Re-evaluate your decision based on the current view.'
                ...
                sentence['value'] = (
                    ...
                    f"{refinement_instruction}"  # 在这里插入精炼指令
...
# === 【修改点 2】: 修改 prepare_data_to_inputs ===
def prepare_data_to_inputs(
    ...
    data_args, 
    target_point, 
    # 新增参数，用于接收上一步的决策，以便传递给preprocess_multimodal
    previous_waypoint_str: Optional[str] = None,
    assist_notice: Optional[str] = None
):
    ori_sources = copy.deepcopy(episodes)
    processor = image_processor
    images = []
    for src in ori_sources[::-1]:
        if 'rgb' in src:
    ...
    conversation_for_human = '<image>\n' + ori_sources[-1]['instruction']
    conversation = [{"from": "human", "value": conversation_for_human}, {"from": "gpt", "value": ""}]
    ...
    for source in ori_sources:
        if 'rgb' not in source.keys(): continue
        deltas.append((np.array(source['sensors']['state']['position']) - pos))
        
    history_waypoint = np.array([(rot.T @ delta) for delta in deltas])
    
    target_point_local = np.array(rot.T @ (target_point - pos))
    x, y = target_point_local[0], target_point_local[1]
    rotation_to_target = rotation_matrix_from_vector(x, y)
    history_waypoint = transform_point(history_waypoint, rotation_to_target)

    delta_vec = history_waypoint[-1] - history_waypoint[-2] if len(history_waypoint) >= 2 else np.array([0, 0, -4.5])
    delta_vec = delta_vec / (np.linalg.norm(delta_vec) + 1e-8)
    delta_str = ','.join([str(round(x, 1)) for x in delta_vec])
    
    cur_pos_vec = history_waypoint[-1]
    cur_pos_str = ','.join([str(round(x, 1)) for x in cur_pos_vec])

    # 将 previous_waypoint_str 参数传递给 preprocess_multimodal
    sources = preprocess_multimodal(
        ...
        delta=delta_str, 
        cur_pos=cur_pos_str,
        previous_waypoint_str=previous_waypoint_str
    )
    
    data_dict = preprocess(sources, tokenizer, has_image=True, prompt=data_args.input_prompt, refine_prompt=data_args.refine_prompt)
    ...
    return data_dict, rotation_to_target, processed_conversations

# (从 inputs_to_batch 到文件末尾的所有函数保持不变)
# ...
