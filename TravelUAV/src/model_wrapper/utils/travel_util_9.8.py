...
def load_model(args):
    ...
    tokenizer, model, image_processor, _ = load_pretrained_model(model_path, args.model_base, model_name, vision_tower=args.vision_tower)   #加载预训练模型，返回对象：文本分词器，LLM 模型,图像预处理器
    smarter_tokenizer_and_embedding_resize(special_tokens_list=['<wp>', '<his>'], tokenizer=tokenizer, model=model)   #扩展 tokenizer 和 embedding，<wp>：表示“waypoint”路径点提示，<his>：表示“history”历史轨迹提示
    model.get_special_token_id({'<wp>': tokenizer.encode('<wp>')[1], '<his>': tokenizer.encode('<his>')[1],   #显式设置特殊 token ID（用于 prompt 构建），告诉模型这些token要重点处理的标记这些 token 会被用于构建指令格式
    ...
    if lora_enable:
        ...
        model = PeftModel.from_pretrained(model, model_path)   #把主模型替换成带 LoRA 模块的结构
        #加载额外微调参数（非 LoRA 模块）
...
def load_traj_model(model_args):
    vision_config = generate_vision_tower_config(model_args.vision_tower, model_args.image_processor)   #构造视觉配置 config 文件路径
    config = transformers.AutoConfig.from_pretrained(vision_config, trust_remote_code=True)   #加载模型配置
    traj_model = VisionTrajectoryGenerator(config)   #以刚才的 config 为参数，实例化轨迹模型
    traj_weights = torch.load(os.path.join(model_args.traj_model_path, 'model_5.pth'), map_location='cpu')   #加载轨迹模型权重文件
    traj_weights = {k: v.to(torch.bfloat16) for k, v in traj_weights.items()}   #转换权重为 bfloat16
...
def smarter_tokenizer_and_embedding_resize(
    ...
    """
    num_new_tokens = tokenizer.add_tokens(special_tokens_list, special_tokens=True)  #把 special_tokens_list 中的 token 添加到 tokenizer 的词表里，告诉 tokenizer 这是“特殊 token”，和普通词不同
    model.resize_token_embeddings(len(tokenizer))   #新 token 后，扩展模型的输入输出 embedding 层
...
def preprocess_multimodal(
    ...
    for source in sources:    # 遍历多个对话输入，每个 source 是一个对话（通常是句子序列）
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value']:   #是否包含图像 token <image>，只有包含 <image> 的句子才会被处理
                sentence['value'] = sentence['value'].replace(DEFAULT_IMAGE_TOKEN, '').strip()   #清理 <image> 占位符
                sentence['prompt'] = copy.deepcopy(sentence['value'])   #把用户原始输入内容（纯语言指令）保存到 prompt 字段中备用
            ...
            if data_args.mm_use_im_start_end:  #用户是否开启了“使用图像边界 token”的配置
                replace_token = DEFAULT_IM_START_TOKEN + replace_token + DEFAULT_IM_END_TOKEN   #用 <im_start><image><im_end> 替换 <image>
            sentence["value"] = sentence["value"].replace(DEFAULT_IMAGE_TOKEN, replace_token)   #将句子中所有 <image> 替换成 replace_token
...
def prepare_data_to_inputs(episodes, tokenizer, image_processor, data_args, target_point, assist_notice = None):
    ...
    conversation_for_human = '<image>\n' + sources[-1]['instruction']  #从最后一帧中取出人类输入的指令，前面加上 <image> 表明是多模态输入
    ...
    if assist_notice is not None:
        stage = assist_notice   #如果外部提供了阶段提示（如 assist_notice="cruise"），就用它
    else:
        stage = 'cruise' if len(sources) > 20 else 'take off'   #否则根据历史长度简单判断是否为起飞或巡航阶段
    ...
    for source in ori_sources:
        if 'rgb' not in source.keys():
            continue
        deltas.append((np.array(source['sensors']['state']['position']) - pos))   #提取其位置，并减去初始位置 pos，即计算相对位移向量,得到历史路径（世界坐标系下）的 delta 序列
    history_waypoint = np.array([(rot.T @ delta) for delta in deltas])      #delta 从世界坐标系 → UAV 自身坐标系
    rotation_to_target = None
    
    target_point = np.array(rot.T @ (target_point - pos))  #目标点先变成相对坐标，再转为 UAV 本地坐标
    ...
    #计算当前位置与方向 delta
    if len(history_waypoint) >= 2:    #取最新两帧间的方向向量,如果历史不足两帧，默认一个负 z 方向（下降）
    ...
    if 'prompt' in data_dict:
        ...
        prompt = None
    #打包最终输入
    data_dict = dict(input_ids=data_dict["input_ids"][0],                       #提取 token 序列和训练标签    
    ...
    return data_dict, rotation_to_target, processed_conversations     #多模态 LLM 模型的全部输入;将当前位置朝向旋转对齐到目标点方向的旋转矩阵
...
def inputs_to_batch(tokenizer, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'prompt' in instances[0]:        
            batch['prompts'] = [instance['prompt'] for instance in instances]    #收集所有样本的 prompt 字符串，用于调试或生成记录
...
def preprocess(
    ...
    """
    if conversation_lib.default_conversation.version.startswith("imgsp_uav"):       #如果当前的对话模板版本是 imgsp_uav 开头的，直接交由特殊处理函数 preprocess_imgsp_uav() 去处理。
    ...
    return dict(input_ids=input_ids, labels=targets)   #模型的输入 token ,模型要预测的目标 token（非human部分）
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if role==conv.roles[0]:   #如果是人类发言，提取原始提示内容
                ...
                if refine_prompt:   #如果启用 refine_prompt，则只保留“目标描述”，组成一个更清晰的任务指令
        ...
        conversations.append(conv.get_prompt())   #将当前完整对话（多轮拼接好）加入总列表
